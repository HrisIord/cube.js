...
export class DevServer {
  ...
  protected dashboardAppProcess:
    | (ChildProcess & { dashboardUrlPromise?: Promise<any> })
    | null = null;
  ...
  public initDevEnv(
    ...
    options: ServerCoreInitializedOptions
  ) {
    ...
    const cubejsToken = jwt.sign({}, options.apiSecret || "secret", {
      expiresIn: "1d",
    });
    ...
    if (
      ...
      console.log(
        "⚠️  Your pre-aggregations will be on an external database. It is recommended to use Cube Store for optimal performance"
      );
    }
    ...
    const catchErrors = (handler) => async (req, res, next) => {
      try {
        ...
        this.cubejsServer.event("Dev Server Error", {
          error: ((e as Error).stack || e).toString(),
        });
      ...
      catchErrors((req, res) => {
        ...
        res.json({
          ...
          shouldStartConnectionWizardFlow:
            !this.options.isReadyForQueryProcessing(),
        ...
        res.json({
          files: files.map((f) => ({
            ...
            absPath: path.resolve(
              path.join(this.cubejsServer.repository.localPath(), f.fileName)
            ),
          })),
        });
      })
    ...
    app.post(
      ...
      catchErrors(async (req, res) => {
        ...
        const tablesSchema =
          req.body.tablesSchema || (await driver.tablesSchema());
        ...
        const files = scaffoldingTemplate.generateFilesByTableNames(
          req.body.tables,
          { dataSource }
        );
        ...
        await Promise.all(
          files.map((file) =>
            fs.writeFile(path.join(schemaPath, file.fileName), file.content)
          )
        );
        res.json({ files });
      })
    ...
    app.get(
      ...
      catchErrors(async (req, res) => {
        ...
        if (
          !fs.pathExistsSync(options.dashboardAppPath) ||
          fs.readdirSync(options.dashboardAppPath).length === 0
        ) {
          res.status(404).json({
            error: `Dashboard app not found in '${path.resolve(
              options.dashboardAppPath
            )}' directory`,
          });
        ...
        if (!fs.pathExistsSync(sourcePath)) {
          res.status(404).json({
            error: `Dashboard app corrupted. Please remove '${path.resolve(
              options.dashboardAppPath
        ...
        res.json({
          status: "created",
          installedTemplates: AppContainer.getPackageVersions(
            options.dashboardAppPath
          ),
        });
      })
    ...
    app.get(
      ...
      catchErrors(async (req, res) => {
        ...
        if (!this.dashboardAppProcess) {
          ...
          this.dashboardAppProcess.dashboardUrlPromise = new Promise(
            (resolve) => {
          ...
          this.dashboardAppProcess.on("close", (exitCode) => {
            if (exitCode !== 0) {
              console.log(
                `Dashboard react-app failed with exit code ${exitCode}`
              );
              this.cubejsServer.event("Dev Server Dashboard App Failed", {
                exitCode,
              });
            }
    ...
    app.get(
      "/playground/dashboard-app-status",
      catchErrors(async (req, res) => {
        this.cubejsServer.event("Dev Server Dashboard App Status");
        const dashboardPort =
          this.dashboardAppProcess &&
          (await this.dashboardAppProcess.dashboardUrlPromise);
        res.json({
    ...
    app.post(
      ...
      catchErrors((req, res) => {
        ...
        if (!DriverDependencies[driver]) {
          return res
            .status(400)
            .json(`'${driver}' driver dependency not found`);
        }
    ...
    app.post(
      "/playground/apply-template-packages",
      catchErrors(async (req, res) => {
        this.cubejsServer.event("Dev Server Download Template Packages");

        const fetcher = process.env.TEST_TEMPLATES
          ? new DevPackageFetcher(repo)
          : new PackageFetcher(repo);
        ...
        const applyTemplates = async () => {
          ...
          if (typeof toApply === "string") {
            const template = manifestJson.templates.find(
              ({ name }) => name === toApply
            );
        ...
        if (this.applyTemplatePackagesPromise) {
          this.applyTemplatePackagesPromise =
            this.applyTemplatePackagesPromise.then(applyTemplates);
        } else {
    ...
    app.get(
      "/playground/live-preview/start/:token",
      catchErrors(async (req: Request, res: Response) => {
    ...
    app.get(
      "/playground/live-preview/status",
      catchErrors(async (req, res) => {
    ...
    app.post(
      "/playground/live-preview/token",
      catchErrors(async (req, res) => {
        const token = await this.livePreviewWatcher.createTokenWithPayload(
          req.body
        );
        res.json({ token });
      })
    ...
    app.post(
      ...
      catchErrors(async (req, res) => {
        ...
        const jwtOptions =
          typeof payload.exp != null ? {} : { expiresIn: "1d" };
    ...
    app.post(
      ...
      catchErrors(async (req: Request, res: Response) => {
        ...
        const schemaConverter = new CubeSchemaConverter(
          this.cubejsServer.repository,
          [
        ...
        schemaConverter
          .getSourceFiles()
          .forEach(({ cubeName: currentCubeName, fileName, source }) => {
            if (currentCubeName === cubeName) {
              this.cubejsServer.repository.writeDataSchemaFile(
                fileName,
                source
              );
            }
